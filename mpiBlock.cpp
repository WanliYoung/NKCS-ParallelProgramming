...
int main(int argc, char* argv[]) {
	...
	int part = QueryNum / numprocs;
	int begin = myid * part;
	int end = begin + part - 1;
	...
	for (int i = begin;i <= end;i++) {
		...
		cout << "result_size = " << result.size() << "from Process " << myid << "of " << numprocs << "on " << processor_name << endl;
	}
	gettimeofday(&t2, NULL);
	timeuse += (t2.tv_sec - t1.tv_sec) * 1000000 + t2.tv_usec - t1.tv_usec;
	cout << "time_use = " << timeuse << "from Process " << myid << "of " << numprocs << "on " << processor_name << endl;
