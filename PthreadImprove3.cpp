...
bitset<25214976>* resultbits; //结果向量首地址
...
void* toresult(void* parm) {
	//baggage* pack = ((baggage*)parm);
	long addr = (long)resultbits;
	long myaddr = addr + 196992 * ((baggage*)parm)->rank;
	for (int j = 0;j < 6156;j++) {
		...
		//如果不为0再去底层判断
			...
			for (int k = 0;k < 256;k++) {
				if ((*resultbits)[(((baggage*)parm)->rank)*1575936+j * 256 + k] == 1) {
					((baggage*)parm)->myresult.push_back((((baggage*)parm)->rank) * 1575936 + j * 256 + k);
				}
			}
		}
	}
...
int main() {
	...
	for (int i = 0;i < QueryNum;i++) {
		...
		//结果转换优化
		resultbits = &packages[0].first;
		int thread_countr = 16;
		pthread_t* thread_handler;
		thread_handler = new pthread_t[thread_countr];
		baggage bags[16];
		for (thread = 0;thread < thread_countr;thread++) {
			bags[thread].rank = thread;
		}
		for (thread = 0;thread < thread_countr;thread++) {
			pthread_create(&thread_handler[thread], NULL, toresult, (void*)&(bags[thread]));
		}
		for (thread = 0;thread < thread_countr;thread++) {
		...
		//进行结果列表的拼接
		for (int i = 0;i < 16;i++) {
			for (int j = 0;j < bags[i].myresult.size();j++) {
				result.push_back(bags[i].myresult[j]);
			}
		}
		//处理完一次查询
