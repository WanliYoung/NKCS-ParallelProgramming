...
#pragma omp parallel for num_threads(8) schedule(dynamic,1539)
		...
		vector<unsignd int> myresult
		//结果转换优化，list[0]就是结果位向量，256为单位判断
		gettimeofday(&t1, NULL);
		long address = (long)&lists[0];
#pragma omp parallel for num_threads(8) private(myresult)
		...
		//结果合并
		for (int j = 0;j < 8;j++) {
			result.insert(result.end(), myresult[j].begin(), myresult[j].end());
		}
