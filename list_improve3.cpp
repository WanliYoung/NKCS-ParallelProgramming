...
bool find(unsigned int e, _INDEX list) { //进行二分查找
	...
	int mid = 0;
	while (low <= high) {
		if (e > list.arr[high] || e < list.arr[low]) {
                        return false;
        	}
//		mid = (low + high) / 2;
		mid = low + (e - list.arr[low])*(high-low) / (list.arr[high] - list.arr[low]);
//cout<<"下标："<<low<<" "<<mid<<" "<<high<<endl;
//cout<<"值"<<list.arr[low]<<" "<<e<<" "<<list.arr[high]<<endl;
		if (e == list.arr[mid]) {
			return true;
		}
